...
//We create two structs with the methods to write and read from the shared channel of []bytes. String is not allowed, so we assume that slice of bytes is a word.
...
//We used approx 5 hours to code version with no goroutines, and one extra hour to code version that u see
